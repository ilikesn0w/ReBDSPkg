  ...
  )
{
  ...
  // ASSERT_EFI_ERROR (Status); < do NOT assert if not found, for some old FW's without normal HII
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "REBDS: HII database not available (%r)\n", Status));
    gHiiDatabase = NULL;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "REBDS: HII CFGROUT not available (%r)\n", Status));
